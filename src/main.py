...
cache_index = (train['contract_time']==0) | (train['many_over_bill']==1)
cache = train[cache_index]
cache = cache.reset_index(drop=True)
print(len(train))
columns = train.columns.tolist()
# pre_columns = ['user_id','current_service','month_traffic','1_total_fee','2_total_fee','3_total_fee','4_total_fee']
rand_index = np.random.randint(len(cache),size=50000)
pre_columns = ['user_id','current_service','month_traffic','2_total_fee','3_total_fee','4_total_fee','1_total_fee']
pre_columns.extend(columns[7:])
tmp = cache.loc[rand_index,:]
tmp = tmp[pre_columns]
tmp.columns = columns
train = train.append(tmp,ignore_index=True)

rand_index = np.random.randint(len(cache),size=50000)
pre_columns = ['user_id','current_service','month_traffic','3_total_fee','4_total_fee','1_total_fee','2_total_fee']
pre_columns.extend(columns[7:])
tmp = cache.loc[rand_index,:]
tmp = tmp[pre_columns]
tmp.columns = columns
...
rand_index = np.random.randint(len(cache),size=50000)
pre_columns = ['user_id','current_service','month_traffic','4_total_fee','1_total_fee','2_total_fee','3_total_fee',]
pre_columns.extend(columns[7:])
tmp = cache.loc[rand_index,:]
tmp = tmp[pre_columns]
tmp.columns = columns
